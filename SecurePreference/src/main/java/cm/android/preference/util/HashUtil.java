...
public class HashUtil {
    ...
    public static SecretKey generateHash(char[] password, byte[] salt) throws InvalidKeySpecException, NoSuchAlgorithmException {
        ...
        try {
            ...
            key = generatePBEKey(password, salt, ALG_PBK, ITERATIONS, KEY_SIZE);
        } catch (NoSuchAlgorithmException e) {
            try {
                key = generatePBEKey(password, salt, ALG_PBE, ITERATIONS, KEY_SIZE);
                ...
                key = generatePBEKey(password, salt,
                        ALG_PBE_LOW, ITERATIONS, KEY_SIZE);
            }
        }
        return key;
    }
